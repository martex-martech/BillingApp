...
const Sidebar: React.FC = () => {
  ...
  return (
    <div
      className={`bg-dark text-white d-flex flex-column vh-100 shadow-sm sidebar ${
        isOpen ? 'w-72' : 'w-16'
    ...
    >
      ...
      <div className="d-flex justify-content-between align-items-center px-4 py-3 border-bottom border-secondary">
        <h4 className="mb-0 fw-bold d-flex align-items-center">
          {isOpen && <span>Ezo Billing</span>}
        </h4>
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="btn btn-outline-light p-1 d-flex align-items-center justify-content-center toggle-button"
          style={{ fontSize: '1rem', width: '32px', height: '32px' }}
        >
          {icons.toggle}
        </button>
      </div>
      ...
      <div className="px-3 py-3 border-top border-secondary">
        ...
        >
          {icons.logout} <span className="menu-label">{isOpen && 'Logout'}</span>
        </button>
      </div>

      {/* Smooth collapse/expand styles */}
      <style>{`
        .nav-link.transition {
